...
response.logo = A(B('Quizz'),_href="http://veron.univ-tln.fr")
response.title = request.application.replace('_',' ').title()

response.menu = [
    (T('Home'), False, URL('default', 'index'), []),
    ('Catégorie-Questions', False, URL('default', 'manage_categories',args=['question']), []),
    ('Banque de questions', False, URL('default', 'manage', args=['quizz_banque']), []),
    ('Catégorie-Quizz', False, URL('default', 'manage_categories', args=['quizz']), []),
    ('Quizz', False, URL('default', 'manage', args=['quizz_quizz']), []),
    ('Etudiants', False, URL('default', 'students'), [])
]

# ----------------------------------------------------------------------------------------------------------------------
# provide shortcuts for development. you can remove everything below in production
# ----------------------------------------------------------------------------------------------------------------------
configuration['app']['production']= 'True'
#if not configuration.get('app.production'):
if not configuration['app']['production']:
    _app = request.application
    response.menu += [
        (str(configuration.get('app.production')), False, URL('admin', 'default', 'site')),
        (T('This App'), False, '#', [
